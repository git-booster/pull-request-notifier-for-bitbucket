...
window.addEventListener("load", function load(event) {
    ...
    define('plugin/prnfb/pr-triggerbutton', [
        ...
        // * BEGIN:  SET OBSERVER TO PUT PR-BUTTON BACK IF IT DISAPPEARS (added in 2025)
        function mutate(mutations) {
            mutations.forEach(function (mutation) {
                var prNotifierActual = document.getElementById('prNotifierActualButton');
                if (!prNotifierActual) {
                    // pr-button not present... let's add it!
                    createPrButton();
                    loadSettingsAndShowButtons();
                }
            });
        ...
        observer.observe(target, prStateChangedConfig);
        // * END:  SET OBSERVER TO PUT PR-BUTTON BACK IF IT DISAPPEARS
