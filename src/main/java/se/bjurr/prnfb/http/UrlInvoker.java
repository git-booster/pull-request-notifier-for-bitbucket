...
public class UrlInvoker {
    ...
    public enum HTTP_METHOD {
        ...
        public static HTTP_METHOD fromObject(Object o) {
            String s = o != null ? String.valueOf(o).trim() : null;
            if (s == null || "".equals(s)) {
                return null;
            } else {
                return HTTP_METHOD.valueOf(s.toUpperCase(Locale.ROOT));
            }
        }
    }
    ...
    void addHeaders(final HttpRequestBase httpRequestBase) {
        for (final PrnfbHeader header : this.headers) {
            ...
            v = v != null ? v.trim() : "";
            if (!"".equals(n)) {
                if (n.equals(AUTHORIZATION)) {
                    LOG.debug("header: \"" + n + "\" value: \"**********\"");
                } else {
                    LOG.debug("header: \"" + n + "\" value: \"" + header.getValue() + "\"");
                }

                httpRequestBase.addHeader(n, v);
            }
        }
    }
