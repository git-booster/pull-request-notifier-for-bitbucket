...
/**
 * Utility for converting back and forth between Java objects (Map, Collection, String, Number,
 * Boolean, null) and JSON.
 */
public class Java2Json {

    public interface _2JS {
        Map<String, Object> _2js();
    }

    public static String _2ListJsonString(List<Object> list) {
        Object o = _2List(list);
        return format(o);
    }

    public static List<Object> _2List(List<Object> list) {
        if (list == null) {
            return null;
        ...
        for (Object o : list) {
            if (o instanceof _2JS) {
                _2JS j = (_2JS) o;
                Map<String, Object> m = j._2js();
                if (m != null) {
                    jsonList.add(j._2js());
                }
            } else {
                jsonList.add(String.valueOf(o));
            }
        }
        return jsonList;
    }
    ...
    public static boolean endsWithJsonSuffix(Path path) {
        if (path != null) {
            final String n = path.getFileName().toString();
            if (n.length() > 5) {
                String suffix = n.substring(n.length() - 5);
    ...
    /**
     * Converts a String of JSON into a Java representation, parsing the result into a structure of
     * nested Map, List, Boolean, Long, Double, String and null objects.
     *
     * @param json String to parse
     * @return A Java representation of the parsed JSON String based on java.util.Map, java.util.List,
     * java.lang.Boolean, java.lang.Number, java.lang.String and null.
     */
    public static Object parse(String json) {
        ...
        try {
            ...
            if (p.pos > 10) {
                // println("NEAR: [" + new String(c, p.pos - 10, Math.min(10 + charsLeft, 20)));
            } else {
                // println("NEAR: [" + new String(c, 0, Math.min(10 + charsLeft, Math.min(20, c.length))));
            }
            throw re;
        }
    }
     ...
     *
     * <p>Expects the Java object to be a java.util.Map, java.util.Iterable, java.lang.String,
     * java.lang.Number, java.lang.Boolean, or null, or nested structure of the above. All other
     * object types cause a RuntimeException to be thrown.
     *
    ...
    private static boolean hasNextItem(Java2Json p, char closingBracket) {
        ...
        if (c == closingBracket) {
            ...
            throw new RuntimeException(
                    "expected whitespace or comma or " + closingBracket + " but found: " + c);
        }
    }
    ...
    private static boolean nextChar(Java2Json p, char charToFind, boolean doThrow) {
        ...
        throw new RuntimeException(
                "Never found " + charToFind + " context=" + new String(p.json, offset, count));
    }
                    ...
                    if (c == '\\') {
                        ...
                        if (Character.isISOControl(c)) {
                            ...
                            if ("7f".equalsIgnoreCase(hex.toString())) {
                                ...
                                if (!tolerant) {
                                    throw new RuntimeException(
                                            "control characters in string literal must be escaped: \\u" + hex);
                                } else {
                            ...
                            for (int j = 0; j < 4; j++) {
                                try {
                                    ...
                                    if (isHex(hexChar)) {
                                        ...
                                        throw new RuntimeException(
                                                "invalid \\u encoded character (must be hex): " + hexChar);
                                    }
                                } catch (ArrayIndexOutOfBoundsException aioobe) {
