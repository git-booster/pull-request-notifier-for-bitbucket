...
public class SettingsService {
    ...
    public void setPrnfbSettingsData(PrnfbSettingsData prnfbSettingsData) {
        inSynchronizedTransaction(
                new TransactionCallback<Void>() {
                    @Override
                    public Void doInTransaction() {
                        PrnfbSettings oldSettings = doGetPrnfbSettings(true);
                        PrnfbSettings newSettings = prnfbSettingsBuilder(oldSettings).setPrnfbSettingsData(
                                prnfbSettingsData
                        ).build();
                        doSetPrnfbSettings(newSettings);
                        return null;
                    }
                }
        );
    ...
    private PrnfbButton doAddOrUpdateButton(PrnfbButton prnfbButton) {
        ...
        PrnfbSettings originalSettings = doGetPrnfbSettings(true);
        PrnfbSettings updated = prnfbSettingsBuilder(originalSettings).withButton(prnfbButton).build();
        doSetPrnfbSettings(updated);
        return prnfbButton;
    }

    private PrnfbNotification doAddOrUpdateNotification(PrnfbNotification newNotification) throws ValidationException {
        ...
        final String proxyPassword = keepIfUnchanged(newNotification.getProxyPassword(), oldProxyPassword);
        newNotification = prnfbNotificationBuilder(newNotification)
                .withUser(user)
        ...
        final PrnfbSettings updated = prnfbSettingsBuilder(originalSettings)
    ...
    private void doDeleteNotification(UUID uuid) {
        final PrnfbSettings originalSettings = doGetPrnfbSettings(true);
        final List<PrnfbNotification> keep = newListWithoutUuid(originalSettings.getNotifications(), uuid);
        final PrnfbSettings withoutDeleted = prnfbSettingsBuilder(originalSettings).setNotifications(keep).build();
        doSetPrnfbSettings(withoutDeleted);
    }
    ...
    private PrnfbSettings doGetPrnfbSettings(boolean forceRead) {
            ...
            synchronized (lock) {
                if (now >= nextCacheExpiry || forceRead) {
                    ...
                    if (s != null) {
                        ...
                        try {
                            Map<String, Object> m = Java2Json.parseToMap(s);
                            cachedSettings = PrnfbSettings._fjs(m);
                        } catch (Exception e) {
                            throw new RuntimeException(
        ...
        if (cachedSettings == null) {
            ...
            return prnfbSettingsBuilder().setPrnfbSettingsData(
                    prnfbSettingsDataBuilder().setAdminRestriction(USER_LEVEL.ADMIN).build()
            ).build();
        } else {
    ...
    private void doSetPrnfbSettings(PrnfbSettings newSettings) {
        ...
        final String keyStorePassword = keepIfUnchanged(
                newSettingsData.getKeyStorePassword(), oldSettingsData.getKeyStorePassword()
        );

        final PrnfbSettingsData adjustedSettingsData = prnfbSettingsDataBuilder(newSettingsData)
                .setKeyStorePassword(keyStorePassword).build();
        final PrnfbSettings adjustedSettings = prnfbSettingsBuilder(newSettings)
                .setPrnfbSettingsData(adjustedSettingsData).build();
        try {
            final String data = Java2Json.format(adjustedSettings._2js());
            final Map<String, Object> m = Java2Json.parseToMap(data);
            final PrnfbSettings adjustedSettingsReparsed = PrnfbSettings._fjs(m);
