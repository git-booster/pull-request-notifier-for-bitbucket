...
public class VariablesContext {
    ...
    public Map<PrnfbVariable, String> getVariables() {
        final Map<PrnfbVariable, String> vars = new HashMap<>();

        if (groups != null) {
            vars.put(PULL_REQUEST_USER_GROUPS, String.join(",", groups));
        }

        if (button != null) {
            vars.put(BUTTON_TRIGGER_TITLE, button.getName());
        }

        if (!isNullOrEmpty(formData)) {
            vars.put(BUTTON_FORM_DATA, formData);
        }

        if (pullRequestEvent != null) {
            if (pullRequestEvent instanceof PullRequestCommentEvent) {
                final PullRequestCommentEvent pullRequestCommentEvent = (PullRequestCommentEvent) pullRequestEvent;
                vars.put(PULL_REQUEST_COMMENT_TEXT, pullRequestCommentEvent.getComment().getText());
                vars.put(PULL_REQUEST_COMMENT_ACTION, pullRequestCommentEvent.getCommentAction().name());
                vars.put(PULL_REQUEST_COMMENT_ID, pullRequestCommentEvent.getComment().getId() + "");
            } else if (pullRequestEvent instanceof PullRequestRescopedEvent) {
                final PullRequestRescopedEvent pullRequestRescopedEvent = (PullRequestRescopedEvent) pullRequestEvent;
                vars.put(PULL_REQUEST_PREVIOUS_FROM_HASH, pullRequestRescopedEvent.getPreviousFromHash());
                vars.put(PULL_REQUEST_PREVIOUS_TO_HASH, pullRequestRescopedEvent.getPreviousToHash());
            } else if (pullRequestEvent instanceof PullRequestMergedEvent) {
                vars.put(PULL_REQUEST_MERGE_COMMIT, ((PullRequestMergedEvent) pullRequestEvent).getCommit().getId());
            }
        }
        return vars;
    }
